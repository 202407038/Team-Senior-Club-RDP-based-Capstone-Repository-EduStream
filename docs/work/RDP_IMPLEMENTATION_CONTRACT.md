...
2026-09-20 후속: #48 공통 상태/메시지 계약과 #49 파일 서비스가 main에 반영됐습니다. [신규 인계 기준](./FINAL_CORE_FILE_HANDOFF.md)을 참조하며, 아래 단방향 native 계약은 그대로 유지합니다. 보안 채널/인증·제어 wire 협의와 신규 RDP/native 소비 구현은 미완료입니다.

계약 확정일: 2026-09-09. 구현/검증 갱신일: 2026-09-15, 코드 기준 main `ec994c6`.

2026-09-18 주의: 아래는 **현재 구현된 단방향 공유 계약**입니다. [확정 UI·피드백](./UI_FEEDBACK_SPEC.md)의 방 비밀번호·자동 복귀·학생 화면/교수자 입력 제어·판서는 후속 신규 범위이며 아직 구현된 계약이 아닙니다. [역할 재배치](./UI_RDP_WORK_ALLOCATION.md)에 따라 1번이 방향별 권한·보호된 인증/초대 전달·수명·파일 요청 계약을 먼저 정한 뒤 이 문서를 갱신합니다. 현재 보기 전용 제한을 단순 제거해 학생→교수자 제어를 허용하지 않습니다. 이 문서의 이전 역할 설명보다 후속 작업에는 새 배분을 우선합니다.
