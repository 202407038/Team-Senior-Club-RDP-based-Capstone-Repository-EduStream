...
현재 main의 RDP 버튼은 원격 접속 미리보기 실험용이며, 학생에게 전송하는 화면은 PNG 주기 전송입니다. 2026-09-09 확정한 최종 목표는 RDP 기반 교수자 화면의 다중 학생 공유이며 9월 필수 구현 범위입니다. 현재 버튼 실행만으로 RDP 공유 완료로 판단하지 않습니다. 구현 후 실제 설치·접속·공유 절차를 이 가이드에 반영합니다.
